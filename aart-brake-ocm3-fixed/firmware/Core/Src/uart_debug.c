...
static void process_command(const char *line)
{
    ...
    if (strncmp(line, "GET", 3) == 0) {
        ...
        /* Find the value — skip to last space                */
...
void usart1_isr(void)
{
    ...
    if ((USART_ISR(USART1) & USART_ISR_TXE_TXFNF) &&
        (USART_CR1(USART1) & USART_CR1_TXEIE)) {
    ...
    if (USART_ISR(USART1) & USART_ISR_RXNE_RXFNE) {
...
void uart_debug_init(void)
{
    ...
    /* Banner                                                 */
    tx_str("\r\n#AART Brake Module v5 — UART debug active\r\n");
...
void uart_debug_tick(const BrakeCtx_t *ctx)
{
    ...
    tx_enqueue(ctx->brake_active ? '1' : '0');
    tx_str("\r\n");
}

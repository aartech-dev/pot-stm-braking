 ...
 *  DAC2 (PA5, DAC_OUT2) -> Q1 anti-brake CURRENT SOURCE (3A).
...
/* 'level' is the 0..4095 logical KA scale used everywhere else (the     */
/* ka_dac_table[], the SET protocol, the floor_mv estimate, the          */
/* display). It is written straight to DAC2 (DAC_OUT2, PA5) as a 12-bit*/
/* code; the R_KA1/R_KA2 divider scales it to 0..0.30V and GD3 refers it */
/* to the WHITE rail for GD1. Code 0 = 0A (off); KA_DAC_MAX = ~3A.       */
static void set_ka_dac(uint16_t level)
{
    if (level > 4095U) level = 4095U;
    dac_load_data_buffer_single(DAC1, level, DAC_ALIGN_RIGHT, DAC_KA_CHAN);
    dac_software_trigger(DAC1, DAC_KA_CHAN);
    s_ctx.ka_dac = level;         /* store logical level (0..4095)        */
}
...
{
    /* NOTE: Q1 is now the TIP147 current source (PWM demand), not a
     * P-ch gate. This floor_mv mapping is the legacy P-ch approximation
     * and should be re-derived from the current-source transfer
     * (I = demand/Rsense) x load. Kept as a coarse threshold for now.  */
...
static void led2_on(void)  { gpio_set(LED2_PORT, LED2_PIN); }
static void led2_off(void) { gpio_clear(LED2_PORT, LED2_PIN); }
static uint32_t s_led2_last_ms = 0U;
static bool     s_led2_on      = false;
...
/* Status LEDs (feature G): green PB5 = drive/keep-alive, red PB6 =
 * brake/fault. A save-acknowledge blink on red takes priority on PB6. */
static void status_led_update(void)
{
    if (s_led_blinks) { led_blink_tick(); led2_off(); return; }
    switch (s_ctx.state) {
    case STATE_BRAKING:
    case STATE_RAMP_IN:
    case STATE_LATVIAN_BRAKE:
        led_on(); led2_off();                 /* red: braking          */
        break;
    case STATE_KEEPALIVE_ONLY:
    case STATE_KA_TUNING:
        led_off();                            /* green slow blink: KA  */
        if ((now_ms() - s_led2_last_ms) >= LED2_BLINK_MS) {
            s_led2_last_ms = now_ms();
            s_led2_on = !s_led2_on;
        }
        if (s_led2_on) led2_on(); else led2_off();
        break;
    case STATE_RELEASE:
    case STATE_PASSTHROUGH:
    default:
        led_off(); led2_on();                 /* green steady: drive   */
        break;
    }
}
...
static void gpio_setup(void)
{
    ...
    /* PA5 = DAC_OUT2 (analog) — anti-brake current demand      */
    ...
    /* PA6 = LM311 reverse-detect (reverse-current / controller-brake flag) input */
    gpio_mode_setup(IDIR_PORT, GPIO_MODE_INPUT, GPIO_PUPD_PULLDOWN, IDIR_PIN);

    /* PB6 status LED (red)                                      */
    gpio_mode_setup(LED_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, LED_PIN);
    led_off();
    /* PB5 second status LED (green) — feature G                 */
...
static void dac_setup(void)
{
    ...
    /* DAC1 CH1 = brake gate (PA4); CH2 = anti-brake demand (PA5). */
    ...
    /* Safe defaults: brake off (0V), anti-brake off (0V).        */
    set_brake_dac(DAC_BRAKE_OFF);
    set_ka_dac(KA_DAC_OFF);
}

/* ── TM1637 4-digit display (PB4=CLK, PB8=DIO, bit-banged) ──
 * The MCU drives the exact digits over the 2-wire TM1637 protocol,
 * so there is no analog proxy: Mode A/B shows the WHITE rail (XX.X V),
 * Mode C shows the live keep-alive floor (X.XX V). Lines are open-drain
 * with the module's pull-ups: gpio_set = release high, gpio_clear =
 * drive low. TM1637 has no minimum clock, so a slow bit-bang is safe. */
static const uint8_t TM_SEG[10] = {
...
{
    for (volatile uint32_t i = 0; i < 100U; i++) { __asm__ volatile("nop"); }
}
static inline void tm_clk(bool hi)
{
    if (hi) gpio_set(TM1637_PORT, TM1637_CLK_PIN);
    else    gpio_clear(TM1637_PORT, TM1637_CLK_PIN);
}
static inline void tm_dio(bool hi)
{
    if (hi) gpio_set(TM1637_PORT, TM1637_DIO_PIN);
    else    gpio_clear(TM1637_PORT, TM1637_DIO_PIN);
}
static void tm_start(void)
{
    tm_dio(true); tm_clk(true); tm_delay();
    tm_dio(false); tm_delay(); tm_clk(false); tm_delay();
}
static void tm_stop(void)
{
    tm_clk(false); tm_dio(false); tm_delay();
    tm_clk(true);  tm_delay(); tm_dio(true); tm_delay();
}
static void tm_write(uint8_t b)
{
    for (uint8_t i = 0; i < 8U; i++) {
        tm_clk(false); tm_delay();
        tm_dio((b & 1U) != 0U); tm_delay();
        tm_clk(true);  tm_delay();
        b >>= 1U;
    }
    /* 9th clock = ACK; release DIO (module pulls it low). */
    tm_clk(false); tm_dio(true); tm_delay();
    tm_clk(true);  tm_delay();
    tm_clk(false); tm_delay();
}
static void tm_show(const uint8_t seg[4])
{
    tm_start(); tm_write(0x40U); tm_stop();            /* auto-increment write */
    tm_start(); tm_write(0xC0U);                       /* start at address 0   */
    for (uint8_t i = 0; i < 4U; i++) tm_write(seg[i]);
    tm_stop();
    tm_start(); tm_write(0x88U | (TM1637_BRIGHTNESS & 7U)); tm_stop(); /* on */
}

static void display_setup(void)
{
    gpio_mode_setup(TM1637_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE,
                    TM1637_CLK_PIN | TM1637_DIO_PIN);
    gpio_set_output_options(TM1637_PORT, GPIO_OTYPE_OD, GPIO_OSPEED_LOW,
                            TM1637_CLK_PIN | TM1637_DIO_PIN);
    gpio_set(TM1637_PORT, TM1637_CLK_PIN | TM1637_DIO_PIN);   /* idle high */
}

/* Render millivolts to 4 digits.
 * decimals=1 → XX.X (rail, DP on units-of-volts digit)
 * decimals=2 → X.XX (floor, DP on the volts digit)            */
static void display_mv(uint16_t mv, uint8_t decimals, uint8_t seg[4])
{
    uint32_t scaled = (decimals == 1U) ? ((mv + 50U) / 100U)   /* deci-volts */
                                       : ((mv + 5U)  / 10U);   /* centi-volts */
    if (scaled > 9999U) scaled = 9999U;
    uint8_t d0 = (uint8_t)(scaled % 10U);
    uint8_t d1 = (uint8_t)((scaled / 10U)  % 10U);
    uint8_t d2 = (uint8_t)((scaled / 100U) % 10U);

    seg[0] = seg[1] = seg[2] = seg[3] = 0x00U;   /* blank */
    if (decimals == 1U) {
        if (d2 > 0U) {                            /* e.g. 14.2 */
            seg[0] = TM_SEG[d2];
            seg[1] = TM_SEG[d1] | TM_DP;
            seg[2] = TM_SEG[d0];
        } else {                                  /* e.g. 4.2 */
            seg[0] = TM_SEG[d1] | TM_DP;
            seg[1] = TM_SEG[d0];
        }
    } else {                                      /* e.g. 0.30 */
        seg[0] = TM_SEG[d2] | TM_DP;
        seg[1] = TM_SEG[d1];
        seg[2] = TM_SEG[d0];
    }
}

/* Throttled (~6 Hz) display refresh; pushes immediately on value change.
 * Mode A/B: WHITE rail (XX.X). Mode C: KA floor proxy (X.XX).
 * DAC2 is direct (higher code → higher floor), so the floor reading is
 * PROPORTIONAL to the code — not inverted as the old PWM proxy did.   */
static void display_update(bool capture_active, uint16_t white_mv,
                           uint16_t preview_dac)
{
    static uint32_t last_ms  = 0U;
    static uint16_t last_val = 0xFFFFU;
    uint32_t now = now_ms();

    uint16_t mv;
    uint8_t  decimals;
    if (capture_active) {
        uint32_t f = ((uint32_t)preview_dac * DISP_FLOOR_FS_MV) / KA_DAC_MAX;
        if (f > DISP_FLOOR_FS_MV) f = DISP_FLOOR_FS_MV;
        mv = (uint16_t)f; decimals = 2U;
    } else {
        mv = (white_mv > DISP_RAIL_FS_MV) ? DISP_RAIL_FS_MV : white_mv;
        decimals = 1U;
    }
    if ((now - last_ms) < 150U && mv == last_val) return;
    last_ms = now; last_val = mv;

    uint8_t seg[4];
    display_mv(mv, decimals, seg);
    tm_show(seg);
}
...
/* ── Reverse-detect / I_DIR (reverse current = controller braking) ── */
/* Asserted = reverse current through the pass switch. Polarity/
 * level CONFIRM vs LM311 output (open-collector pulled to +3V3).  */
static bool idir_asserted(void)
{
    return gpio_get(IDIR_PORT, IDIR_PIN) != 0;   /* active-high assumed */
}

/* ── Brake detection: current-direction + V_BLACK ────────── */
static bool update_brake_active(bool cur, uint16_t black_mv, bool idir)
{
    const DebugParams_t *p = uart_debug_get_params();
    /* Reverse current = controller braking: latch and hold across the
     * brief reverse-detect dropouts near the zero-current threshold. */
    if (idir) { s_ctx.idir_hold = IDIR_HOLD_TICKS; return true; }
    if (s_ctx.idir_hold > 0U) { s_ctx.idir_hold--; return true; }
    /* No reverse current: V_BLACK handles coast/lift-off entry and the
     * forward-drive exit.                                                 */
    if (black_mv < p->brake_enter_mv) return true;   /* coast / lift-off */
    if (black_mv > p->brake_exit_mv)  return false;  /* driving forward  */
    return cur;
}
...
static void flash_load(void)
{
    ...
    if (magic != FLASH_MAGIC) {
        ...
        s_ctx.saved_pot_min      = 0U;
        return;
    }
    /* Read each double-word's low/high 32-bit halves, mirroring
     * flash_save_all() exactly. (Earlier code read the wrong offsets, so
     * only DW0 persisted; corrected here, and DW1's spare high word now
     * carries pot_min/ramp_curve — features D and J.)                    */
    uint32_t d0h = *(volatile uint32_t *)(FLASH_SAVE_ADDR + 0x04U);
    uint16_t ka   = (uint16_t)(d0h & 0xFFFFU);
    uint16_t ramp = (uint16_t)(d0h >> 16U);

    uint32_t d1l = *(volatile uint32_t *)(FLASH_SAVE_ADDR + 0x08U);
    uint16_t b_enter = (uint16_t)(d1l & 0xFFFFU);
    uint16_t b_exit  = (uint16_t)(d1l >> 16U);

    uint32_t d1h = *(volatile uint32_t *)(FLASH_SAVE_ADDR + 0x0CU);
    uint16_t pot_min    = (uint16_t)(d1h & 0xFFFFU);
    uint16_t ramp_curve = (uint16_t)(d1h >> 16U);

    uint32_t d2l = *(volatile uint32_t *)(FLASH_SAVE_ADDR + 0x10U);
    uint16_t b_ohms  = (uint16_t)(d2l & 0xFFFFU);
    uint16_t latvian = (uint16_t)(d2l >> 16U);

    uint32_t d3l = *(volatile uint32_t *)(FLASH_SAVE_ADDR + 0x18U);
    uint16_t rel_a = (uint16_t)(d3l & 0xFFFFU);
    uint32_t d3h = *(volatile uint32_t *)(FLASH_SAVE_ADDR + 0x1CU);
    uint16_t rel_b = (uint16_t)(d3h & 0xFFFFU);
    uint16_t rel_c = (uint16_t)(d3h >> 16U);
    ...
    s_ctx.saved_pot_min      = (pot_min < POT_MIN_MAX)   ? pot_min : 0U;

    uart_debug_load_params(b_enter, b_exit, b_ohms, latvian,
                           rel_a, rel_b, rel_c, ramp_curve);
}
                    ...
                    uint16_t ramp_curve)
{
    ...
    if (pot_min > POT_MIN_MAX)      pot_min  = 0U;
    if (ramp_curve > RAMP_CURVE_LOG) ramp_curve = RAMP_CURVE_DEFAULT;
    ...
    flash_program_double_word(FLASH_SAVE_ADDR + 0x08U,
        ((uint64_t)ramp_curve << 48U) | ((uint64_t)pot_min << 32U) |
        ((uint64_t)b_exit << 16U) | b_enter);
    ...
    s_ctx.saved_pot_min      = pot_min;
}

/* KA auto-save (on leaving Mode C). Note the deliberate asymmetry in the
 * argument sourcing: brake_enter/exit, brake_ohms_idx, latvian and ramp_curve
 * are taken LIVE (p->...) so the auto-save captures the operator's current
 * working values, whereas ramp_ms and release_ms_a/b/c are taken from the
 * last-PERSISTED mirror (s_ctx.saved_*). The latter group only ever commits
 * on an explicit SAVE (ramp_ms is "applied on SAVE" by design), so a KA
 * auto-save must NOT silently persist live-but-unsaved ramp/release edits.
 * This is intentional — do not "fix" it to use p->ramp_ms / p->release_ms_*. */
static void do_save(void)
{
    const DebugParams_t *p = uart_debug_get_params();
    flash_save_all(s_ctx.saved_ka_dac, s_ctx.saved_ramp_ms,
                   p->brake_enter_mv, p->brake_exit_mv,
                   p->brake_ohms_idx, p->latvian_dvdt_mv_per_ms,
...
/* Feature J: capture the current (raw, uncorrected) pot reading as the
 * CCW zero offset and persist it. Triggered explicitly by 'CAL POT_MIN'. */
void brake_calibrate_pot_min(void)
{
    extern volatile uint16_t s_adc_buf[ADC_NUM_CHANNELS];
    uint16_t raw = s_adc_buf[2];
    if (raw < POT_MIN_MAX) {
        s_ctx.saved_pot_min = raw;
        do_save();
        led_blink_start();
    }
}
...
static bool ramp_tick(uint16_t target, uint16_t alpha)
{
    ...
    int32_t step;
    /* Feature D: ramp shape. EXP (default) = fast initial bite via the
     * existing IIR; LINEAR = constant step; LOG = accelerating (slow
     * start). All stateless; tick is ~1ms.                              */
    const DebugParams_t *p = uart_debug_get_params();
    uint16_t rms = p->ramp_ms ? p->ramp_ms : 1U;
    switch (p->ramp_curve) {
    case RAMP_CURVE_LINEAR: {
        int32_t m = (int32_t)(DAC_BRAKE_HARD / rms);
        if (m < 1) m = 1;
        step = (err > 0) ? m : -m;
        break;
    }
    case RAMP_CURVE_LOG: {                 /* slow start, late bite     */
        int32_t m = (int32_t)((uint32_t)cur / rms) + 1;
        step = (err > 0) ? m : -m;
        break;
    }
    case RAMP_CURVE_EXP:                    /* fast initial bite (def.)  */
    default:
        step = (err * (int32_t)alpha) / 256;
        break;
    }
    if (step == 0) step = (err > 0) ? 1 : -1;
    if (err > 0 && step > err) step = err;
    if (err < 0 && step < err) step = err;
...
void brake_tick(void)
{
    ...
    /* Feature J: subtract stored pot CCW zero, rescale to full span  */
    {
        uint16_t raw  = s_adc_buf[2];
        uint16_t pmin = s_ctx.saved_pot_min;
        if (pmin >= POT_MIN_MAX) pmin = 0U;
        if (raw <= pmin)         raw  = 0U;
        else if (pmin > 0U)
            raw = (uint16_t)(((uint32_t)(raw - pmin) * ADC_MAX) / (ADC_MAX - pmin));
        s_ctx.pot_raw = raw;
    }
    ...
    if (s_ctx.mode_sel == MODE_C) {
        /* Pot dials KA floor live; display shows it         */
        ...
        display_update(true, s_ctx.white_mv, ka_dac_table[s_ctx.pot_idx]);
        status_led_update();
        return;
    }
    ...
    /* ── MODE_B: anti-brake table, brushed ────────────── */
    {
        ...
        if (s_ctx.brake_active) {
            /* Braking detected (I_DIR reverse current / V_BLACK collapse):
             * Q2 brakes AND Q1 injects the table anti-brake at the same
             * time, so the pot sets the NET brake (trail brake).          */
            set_ka_dac(ka_tgt);
            s_ctx.brake_dac_target = brk_tgt;
            s_ctx.state = ramp_tick(brk_tgt, alpha)
                          ? STATE_BRAKING : STATE_RAMP_IN;
        } else {
            /* Controller driving/coasting forward: never inject anti-brake
             * while the driver is on throttle.                            */
            set_ka_dac(DAC_KA_OFF);
            if (s_ctx.state == STATE_BRAKING || s_ctx.state == STATE_RAMP_IN)
                s_ctx.state = STATE_RELEASE;
            if (s_ctx.state == STATE_RELEASE) {
    ...
    /* Display: track voltage normally                       */
    display_update(false, s_ctx.white_mv, 0U);
    status_led_update();
}

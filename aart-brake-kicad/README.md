...
```
[Connectors/Inputs] → [MCU U1] → [Gate Drivers GD1/GD2] → [FETs Q1/Q2] → [Track Connectors]
...
**Q1 (brake FET, N-ch low-side):**
TIM3_CH1 uses INVERTED output polarity. CCR=0 → pin HIGH → Q1 fully on → dead short.
The 10kΩ pull-up R2 is on the GD1 *input* (not Q1's gate). When the MCU is unpowered,
R2 holds GD1 input HIGH → Q1 stays on → motor remains shorted = safe default.

**Q2 (keep-alive FET, P-ch high-side):**
IRF9Z34N with gate driven by GD2 (UCC27524). P-channel: gate LOW = FET on.
TIM3_CH2 uses normal polarity: CCR=0 → pin LOW → Q2 off.

**D4 (mode C protection):**
SS34 Schottky in series with Q2 drain output, oriented so current flows
Q2 → D4 → track+. When Q1 hard-brakes (dead short), D4 blocks the reverse
current spike from the track back through Q2's body diode.
...
For Q2 (P-channel), the driver output drives the gate low to turn the FET on.
The gate drive supply for GD1/GD2 is 3.3V (from U2). Verify that 3.3V is
